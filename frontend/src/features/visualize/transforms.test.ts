...
import {
  groupByTier, groupByImpact, groupBySector, groupByImpactLevel, sectorLabel, rankByMove, rankByConfidence,
...
describe('rankByMove', () => {
  it('sorts descending by the midpoint of magnitude_low and magnitude_high', () => {
    ...
    expect(rankByMove([weak, strong, mid]).map((c) => c.company_id)).toEqual([2, 3, 1]);
  });
  ...
  it('keeps input order for equal midpoints (stable sort)', () => {
    ...
    expect(rankByMove([a, b]).map((c) => c.company_id)).toEqual([1, 2]);
  });

  it('returns an empty array for an empty input', () => {
    expect(rankByMove([])).toEqual([]);
  });

  it('returns a single-element array unchanged', () => {
    const only = company({ company_id: 1 });
    expect(rankByMove([only])).toEqual([only]);
  });
});
