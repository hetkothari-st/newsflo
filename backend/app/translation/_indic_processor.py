"""VENDORED, unmodified below the header: IndicProcessor from AI4Bharat's
IndicTransToolkit (github.com/VarunGumma/IndicTransToolkit), taken from the
last pure-Python revision of IndicTransToolkit/processor.py -- the commit
immediately before that file was rewritten as a Cython .pyx.

Why vendored rather than a pip dependency: the current release
(IndicTransToolkit 1.1.1, the only version on PyPI) ships `processor.pyx`
and builds a C extension at install time, which requires MSVC on Windows
and a full toolchain in the Docker image. The pure-Python revision is
functionally the same class with the same public API
(preprocess_batch / postprocess_batch) and no compiler requirement.

It is NOT optional. IndicTrans2 will not produce correct output without it:
the model expects its input already Moses-tokenized, script-normalized, and
prefixed with `<src_lang> <tgt_lang>` tags, and expects its output
detokenized and placeholder-restored. Feeding raw text straight to the
tokenizer silently degrades quality rather than erroring.

Upstream is MIT licensed. Do not edit below this header -- if it needs to
change, re-vendor from upstream so the provenance stays checkable.
"""
import re
