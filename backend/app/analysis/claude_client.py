...
ANALYSIS_INSTRUCTIONS = (
    ...
    "7. Also fill in key_points: 1-4 short bullet fragments (not full "
    ...
    "and the specific precedent, each as its own terse fragment. Each bullet "
    "must state a causal mechanism (why this news affects this company, and "
    "how) -- never any of these three failure patterns: (a) restating a "
    "price/number the article itself already reports as if it were insight "
    "(e.g. a bullet that's just \"Share price up 2.4% ahead of results\" -- "
    "that's the article's own headline, not analysis); (b) a vague sentiment "
    "line with no mechanism (e.g. \"Market anticipates positive surprise\" "
    "says nothing about WHY); (c) a generic, always-true company fact untied "
    "to this specific news (e.g. listing a company's business segments is "
    "not an insight unless it names which segment THIS news actually hits "
    "and how). If you cannot state a real, specific mechanism for a bullet, "
    "drop it rather than padding key_points to reach 4 items -- fewer, "
    "sharper bullets beat more, vaguer ones. It is correct for key_points to "
    "have only 1-2 entries when that's all the genuine, specific mechanism "
    "supports.\n"
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "key_points": {
                                ...
                                "items": {"type": "string"},
                                "description": (
                                    "1-4 short bullet fragments (not full sentences, max ~12 "
                                    "words each) that compress `rationale` into only the "
                                    "essential facts -- for a feed UI where nobody reads a "
                                    "full paragraph per company. Each must state a causal "
                                    "mechanism (why this news affects this company, and how) "
                                    "-- never restate a price/number the article already "
                                    "reports, never a vague sentiment line with no mechanism, "
                                    "never a generic company fact untied to this specific news."
                                ),
                            },
